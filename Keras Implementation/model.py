...
aux_in_full = Lambda(K.one_hot, arguments={
                     'num_classes': n_dims}, output_shape=(None, n_dims))(aux_in)
...
for e in range(n_epoches):
    ...
    model.fit([train_aux_input, train_main_input], [train_y],
    ...
    for i in range(n_batch):
        ...
        nd[i] = nd_metrics(test_main_input[batch_range, input_window_length:, 0],
                           rewritten_input[batch_range, input_window_length:, 0], test_vi[batch_range])
        rmse[i] = rmse_metrics(test_main_input[batch_range, input_window_length:, 0],
                               rewritten_input[batch_range, input_window_length:, 0], test_vi[batch_range])
        if (nd[i] <= 0.09 or rmse[i] <= 0.4 or (nd[i] < 0.11 and rmse[i] < 0.5)):
            plot(test_main_input[i*batch_size:i*batch_size+8, :, 0],
                 rewritten_input[i * batch_size:i*batch_size+8, :, 0],
                 test_vi[i*batch_size:i*batch_size+8], 8, i, e)
            print(nd[i], rmse[i])
