...
train_aux_input =  np.array(data[:N,1:,5], dtype='int32') # the one-hot position
#train_aux_input = (np.arange(n_dims) == train_aux_input[...,None]-1).astype(np.int32, copy=False)
train_y = data[:N,1:,4].reshape(-1, window_length, 1)
...
test_aux_input = np.array(data[N:,1:,5], dtype='int32') # the one-hot position
#test_aux_input = (np.arange(n_dims) == test_aux_input[...,None]-1).astype(int)
test_y = data[N:,1:,4].reshape(-1, window_length, 1)
...
#for i in range( 100,101 ): # just for test
    ...
    for j in range(output_window_length+1):
        ...
        this_batch_predict = model.predict([test_aux_input[i*64:(i+1)*64,:input_window_length + j],
        test_pred[i*64:(i+1)*64,:input_window_length + j,0:4]],batch_size = 64, verbose=0)
    ...
    # print("test_input:")
    # print(test_main_input[i*64:i*64+10, -10:, 0])
    # print("test_pred:")
    # print(test_pred[i*64:i*64+10, -10:, 0])
    ...
    print('nd for this batch: ', nd[i])
    print('rmse for this batch: ', rmse[i])
